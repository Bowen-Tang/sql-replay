...
1. ./sql-replay -mode parse 读取慢查询日志，并输出成格式化的 json 内容
2. ./sql-replay -mode replay 读取格式化后的 json 文件，指定目标库执行 SQL 并行回放（不同连接号并行，相同连接号串行），并将回放结果按连接号输出到不同文件中
3. ./sql-replay -mode load 导入回放结果到数据库的 replay_info 表中
